    ...
    do {
      if (target !== null && task !== null) {
        ...
        if (task === 'withdraw' && target instanceof Creep) {
          if (
            (target.carry[RESOURCE_ENERGY] || 0) > 0 ||
            (creep.carry[RESOURCE_ENERGY] || 0) < creep.carryCapacity
          ) {
...
function findHarvesterToEmpty(creep: Creep, blacklist: MuleTarget[]): Creep|null {
  const allH = creep.room.find<Creep>(FIND_MY_CREEPS, {filter: (c: Creep) => c.memory.role === 'harverstr'});
  const harvesters: Creep[] = _.difference(allH, _.filter(blacklist, (c: Creep|any) => c instanceof Creep));
